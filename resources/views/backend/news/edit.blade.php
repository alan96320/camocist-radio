...
@section('title', 'Camokakis Music Stations' . ' | ' . __('strings.backend.dashboard.news'). ' | Edit')
        ...
        <div class="col">
            <div class="card">
                <div class="card-header">
                    <strong>@lang('strings.backend.dashboard.news') | Edit</strong>
                </div><!--card-header-->
                <div class="card-body">
                    <form method="post" action="{{route('admin.news.update', $news->id)}}" enctype="multipart/form-data">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <input id="title" type="text" class="form-control{{ $errors->has('title') ? ' is-invalid' : '' }}" name="title" value="{{ old('title') ?? $news->title }}" required placeholder="Title">
                                @if ($errors->has('title'))
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <textarea id="description" type="text" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}" name="description">{{ old('description') ?? $news->description }}</textarea>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <input id="link" type="url" class="form-control{{ $errors->has('external_url') ? ' is-invalid' : '' }}" name="external_url" value="{{ old('external_url') ?? $news->external_url }}" required placeholder="Link">
                            ...
                            <div class="col-md-6">
                                <select id="category_id" name="category_id" class="form-control {{ $errors->has('type') ? ' is-invalid' : '' }}" required>
                                  ...
                                  @foreach($categories as $category)
                                    <option {{($category->id == (old('category_id') ?? $news->category_id)) ? 'selected': ''}} value="{{$category->id}}">{{$category->name}}</option>
                                  @endforeach
                                </select>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <div class='input-group date'>
                                    <input id="date" type="text" class="form-control{{ $errors->has('date') ? ' is-invalid' : '' }}" name="date" value="{{ old('date') ?? $news->date }}">
